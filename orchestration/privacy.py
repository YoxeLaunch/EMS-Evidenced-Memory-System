"""Política de egreso de datos — principio 5 de `docs/00-VISION-Y-ARQUITECTURA.md`.
...
sale, y eso incluye los namespaces nuevos. El error que esto evita es el
silencioso: algo sensible empieza a viajar en prompts a un proveedor remoto
porque el `default` era permisivo.
...
class EgressPolicy:
    ...
    def politica_de(self, namespace: str) -> str:
        """Política del namespace exacto, o la del ancestro explícito más
        específico (misma contención de un solo sentido que `_coincide()` en
        `orchestration/permissions.py`), o `por_defecto` si ninguno aplica.
        """
        ns = namespace.rstrip("/")
        mejor: tuple[str, str] | None = None
        for patron, valor in self._namespaces.items():
            p = patron.rstrip("/")
            if ns == p or ns.startswith(p + "/"):
                if mejor is None or len(p) > len(mejor[0]):
                    mejor = (p, valor)
        return mejor[1] if mejor is not None else self.por_defecto
