...
Vendorizado de MagnusAgent (`providers/registry.py`) sin cambios de diseño —
ver `docs/02-COMPONENTES-REUTILIZABLES.md`. Este módulo es el ÚNICO lugar
donde vive la política de failover: ni el motor ni los adaptadores deciden a
quién llamar cuando algo falla.
...
Ante un fallo se intentan, en este orden:
  ...
  1. `primary` del perfil pedido (con reintentos acotados si el error es
     reintentable).
  2. `fallback` interno del perfil — es el sustituto más cercano: mismo
     propósito, otro proveedor.
  3. `fallback_profile` del agente, si el llamante lo pasa — es un cambio de
     perfil, es decir una degradación deliberada de capacidad, y por eso va
     después del sustituto equivalente.
...
class ProviderRegistry:
    ...
    def resolve(self, profile: str) -> ProfileConfig:
        if profile not in self._profiles:
            raise ProfileNotFound(
                ...
                f"Perfiles declarados: {self.profiles()}")
    ...
    def complete_with_trace(
        ...
        `only_local=True` poda del plan cualquier proveedor remoto. Lo usa la
        política de egreso (`orchestration/privacy.py`) cuando el contenido
        recuperado no puede salir del dispositivo: el fallback automático no
        puede convertirse en la puerta trasera por la que sale igualmente.
        """
