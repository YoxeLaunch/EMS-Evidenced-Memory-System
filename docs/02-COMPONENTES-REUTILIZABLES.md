...
Ninguno de estos módulos se importa en tiempo de ejecución — Embudo es un
...
| Componente Magnus | Ruta | Qué hace en Magnus | Plan en Embudo |
|---|---|---|---|
...
| Patrón de `snapshot_id` + hash por chunk | [`kernel/rag/file_store.py`](../../MagnusAgent/kernel/rag/file_store.py) | Permite reproducir una cita: si la nota cambió, se sabe. | **Reusar el patrón, no el código** — en Embudo el equivalente es el hash de `MemoryClaim.id` + la cadena `supersedes`/`superseded_by`, que ya cumple el mismo rol pero con un modelo de versionado distinto (sucesión, no snapshot global). |
...
| `orchestration/memory/memory_engine.py` y `sqlite_memory_engine.py` | [`orchestration/memory/`](../../MagnusAgent/orchestration/memory/) | Esqueleto de memoria, nunca conectado al motor; `recall` usa `LIKE '%query%'`. | **No portar la implementación.** Sirve como referencia de la interfaz que Magnus previó, pero el modelo de datos de `MemoryClaim` (niveles, refuerzo, sucesión, caducidad) es sustancialmente más rico. Construir desde cero en Embudo con esa interfaz en mente para que, si algún día Magnus quiere adoptar memoria real, el puerto ya calce. |
