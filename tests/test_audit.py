...
def test_jsonl_trace_store_escribe_una_linea_por_entrada(tmp_path):
    ...
    [archivo] = list(tmp_path.glob("embudo-*.jsonl"))
    lineas = archivo.read_text(encoding="utf-8").strip().splitlines()
    assert len(lineas) == 2
    assert json.loads(lineas[0])["evento"] == "extraction"
    assert json.loads(lineas[1])["evento"] == "reinforcement"


def test_build_entry_no_incluye_texto_de_conversacion(tmp_path):
    entry = build_entry(agent_id="a1", event="promotion", details={"claim_id": "x"},
                        source_conversation_ids=["conv-1"])
    assert entry["conversaciones_origen"] == ["conv-1"]
    assert "texto" not in entry
    assert set(entry) == {"ts", "agente", "evento", "detalles", "conversaciones_origen"}
...
def test_trace_store_from_env_con_variable_usa_jsonl(monkeypatch, tmp_path):
    monkeypatch.setenv("EMBUDO_TRACE_DIR", str(tmp_path / "traces"))
