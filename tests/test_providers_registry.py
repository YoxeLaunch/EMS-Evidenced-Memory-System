...
from providers.registry import (
    ProfileConfig, ProfileNotFound, ProviderRegistry, ProviderUnavailable,
    ResolvedModel,
)
...
def _profiles() -> dict[str, ProfileConfig]:
    return {
        "perfil_test": ProfileConfig(
            primary=ResolvedModel("fake", "fake-grande", {"temperature": 0.0}),
            fallback=ResolvedModel("fake_alterno", "fake-respaldo"),
        ),
        "perfil_barato": ProfileConfig(
            primary=ResolvedModel("fake", "fake-pequeno"),
        ),
        "perfil_sin_adaptador": ProfileConfig(
            primary=ResolvedModel("proveedor_inexistente", "modelo-x"),
        ),
    }


def _registry(*, retries: int = 0, **providers) -> ProviderRegistry:
    """`retries=0` y un `sleep` que no duerme: ningún test debe pagar un
    backoff real salvo el que prueba explícitamente los reintentos."""
    return ProviderRegistry(providers, _profiles(), retries=retries,
                            backoff_s=0.0, sleep=lambda _s: None)


# -- resolución -----------------------------------------------------------------
def test_resuelve_el_perfil_al_modelo_declarado():
    fake = FakeProvider("fake")
    reg = _registry(fake=fake)
...
def test_perfiles_distintos_resuelven_a_modelos_distintos():
    fake = FakeProvider("fake")
    reg = _registry(fake=fake)
...
def test_un_perfil_inexistente_falla_con_mensaje_claro_no_con_keyerror():
    reg = _registry(fake=FakeProvider("fake"))
...
def test_un_proveedor_sin_adaptador_falla_claro_no_con_keyerror():
    reg = _registry(fake=FakeProvider("fake"))
...
def test_is_available_distingue_perfiles_utilizables():
    reg = _registry(fake=FakeProvider("fake"))
...
def test_un_error_reintentable_activa_el_fallback_del_perfil():
    primario = FakeProvider("fake", fail_with=ProviderError("503", retryable=True))
    respaldo = FakeProvider("fake_alterno")
    reg = _registry(fake=primario, fake_alterno=respaldo)
...
def test_un_error_no_reintentable_no_activa_el_fallback():
...
def test_sin_fallback_declarado_el_error_se_propaga():
    primario = FakeProvider("fake", fail_with=ProviderError("503", retryable=True))
    reg = _registry(fake=primario)

    with pytest.raises(ProviderError):
        reg.complete(_req("perfil_barato"))   # perfil_barato no declara fallback


def test_el_perfil_de_fallback_del_agente_va_despues_del_fallback_del_perfil():
    """Orden: primario → fallback del perfil → fallback_profile del llamante."""
    ...
    assert resp.model == "fake-pequeno"          # el primario del perfil de fallback
    assert trace.attempts[-1].profile == "perfil_barato"


def test_los_reintentos_estan_acotados():
    primario = FakeProvider("fake", fail_with=ProviderError("timeout", retryable=True))
    respaldo = FakeProvider("fake_alterno")
    reg = _registry(retries=2, fake=primario, fake_alterno=respaldo)
...
def test_un_adaptador_que_no_normaliza_su_error_no_se_reintenta():
    ...
    respaldo = FakeProvider("fake_alterno")
    reg = _registry(retries=2, fake=RotoProvider("fake"), fake_alterno=respaldo)
...
def test_only_local_poda_el_plan_a_proveedores_locales():
    reg = _registry(fake=FakeProvider("fake"), fake_alterno=FakeProvider("fake_alterno"),
                    local_providers={"fake_alterno"})

    with pytest.raises(ProviderUnavailable) as exc:
        reg.complete(_req("perfil_barato"), only_local=True)  # primario 'fake' no es local

    assert "egreso remoto" in str(exc.value)


# -- trazabilidad -------------------------------------------------------------------
def test_la_traza_permite_auditar_perfil_modelo_y_fallback():
...
# -- from_yaml ------------------------------------------------------------------------
def test_from_yaml_carga_perfiles_desde_archivo(tmp_path):
    modelos_yaml = tmp_path / "models.yaml"
    modelos_yaml.write_text(
        """
profiles:
  perfil_yaml:
    primary:
      provider: fake
      model: fake-desde-yaml
      temperature: 0.2
""", encoding="utf-8")

    reg = ProviderRegistry.from_yaml(str(modelos_yaml), {"fake": FakeProvider("fake")})
    resp = reg.complete(_req("perfil_yaml"))

    assert resp.model == "fake-desde-yaml"
