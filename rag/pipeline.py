...
Vendorizado de MagnusAgent (`kernel/rag/pipeline.py`) sin cambios de diseño —
ver `docs/02-COMPONENTES-REUTILIZABLES.md`. El contrato ya es exactamente lo
que necesita `MemoryClaim` (`docs/01-MEMORIA-NIVELADA.md`) como documento
indexable: los claims T2/T3 activos son, funcionalmente, el mismo tipo de
`ScoredChunk` que un pasaje de una fuente curada.

Recuperación híbrida (léxica + vectorial local) → fusión de rankings → rerank
opcional → filtro por umbral y por los namespaces autorizados → contexto con
citas trazables.
...
class RAGRequest:
    query: str
    namespaces: list[str]              # parcela autorizada del llamante
    top_k: int = 8
...
class RAGContext:
    ...
    def as_prompt_block(self, max_chars: int | None = None) -> str:
        ...
        `max_chars` acota el tamaño del contexto que se manda al proveedor: se
        cortan chunks ENTEROS desde el final, nunca a mitad de un pasaje — un
        fragmento truncado sigue arrastrando su cita y haría citable algo que
        ya no dice lo que decía.
        """
        parts = []
        total = 0
...
#: Cuántos candidatos pedir a cada retriever por cada hueco de `top_k`. RRF
#: fusiona mejor cuanto más profundas son las listas de entrada (medido en
#: Magnus sobre su wiki real, ver `docs/02-COMPONENTES-REUTILIZABLES.md`).
OVERSAMPLE = 8
...
class RAGPipeline:
    ...
    Fusión de rankings: el **orden** lo decide Reciprocal Rank Fusion sobre
    las dos listas, y el **umbral** se aplica sobre el score original de cada
    retriever. Son dos preguntas distintas:
      ...
      - "¿es este chunk evidencia suficiente?" → el score en su propia escala,
        que es lo que calibra `min_score`.

    Fusionar con `max(score)` entre las dos listas compara magnitudes de
    escalas distintas y hace que el retriever con la escala más generosa
    domine la selección — por eso RRF decide el orden y el score original
    decide el corte.
    """
    ...
    def build_context(self, req: RAGRequest) -> RAGContext:
        ...
        #    resultado en vez de mejorarlo — el retriever más débil desplaza
        #    aciertos del más fuerte sin sobremuestreo.
        ...
        if req.require_citations and not citations:
            # sin evidencia suficiente → el llamante debe declararlo, no alucinar
