"""Embedder local que implementa el puerto `Embedder`.

Portado de MagnusAgent (`kernel/rag/embedder.py`) sin cambios de diseño — ver
`docs/02-COMPONENTES-REUTILIZABLES.md`. Sirve igual para detectar similitud
entre candidatos nuevos y `MemoryClaim` existentes en el paso de
deduplicación del pipeline de memoria nivelada (`docs/01-MEMORIA-NIVELADA.md`).
...
"paro" y "desempleo" siguen sin parecerse — mismo límite documentado y
aceptado en Magnus, mismo trade-off aquí. Lo que sí aporta:

  - **IDF**: un rasgo que aparece en todos los documentos pesa menos.
  - **Bigramas**: "banco central" es un rasgo, no dos palabras sueltas.
  - **Prefijos**: "inflación", "inflacionario" e "inflacionaria" comparten
    rasgo. En español esto importa mucho.
  - **Coseno normalizado**: la longitud del texto deja de sesgar el score.
...
class HashingEmbedder:
    ...
    def _vector(self, texto: str) -> list[float]:
        ...
        # puede traer términos que solo están en un documento).
